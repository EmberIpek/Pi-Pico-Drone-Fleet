...
4/28/2026
Project resumed, new (sturdier) wires soldered onto prototyping board. Pico code will pack and send telemetry data to RPi. Current protocol for data packets is as follows: 1-byte ID, 2-byte packet sequence number, 4-byte HC-SR04 data, 2-byte accelerometer X data, 2-byte accelerometer Y data, 2-byte accelerometer Z data, 2-byte gyroscope X data, 2-byte checksum. RPi will unpack the data and send inputs to drone controller based on data. Pi 5 does not support Debian Bullseye, add os_check=0 in config.txt to override. UDP receive code on Pi successfully unpacks data according to protocol:
